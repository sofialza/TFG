...
const ManejarStock = () => {
  ...
  const handleSimularPedido = async () => {
    ...
    try {
      ...
      // Auto-crear órdenes de compra para ítems con déficit
      const itemsConDeficit = proyeccionEnriquecida.filter(item => item.deficit > 0);
      
      if (itemsConDeficit.length > 0) {
        try {
          const items = itemsConDeficit.map(item => {
            const idProveedor = item.insumo?.provInsumos?.[0]?.proveedor?.idProveedor;
            
            if (!idProveedor) {
              console.warn(`El insumo "${item.nombreInsumo}" no tiene proveedor asignado`);
              return null;
            }
          ...
          if (items.length > 0) {
            ...
            console.log('Órdenes de compra creadas automáticamente');
          }
        } catch (error) {
          console.error('Error creando órdenes automáticas:', error);
        }
      }
    } catch (error) {
